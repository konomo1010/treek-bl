...
bool CheckLongEntrySignal(double &high[], double &low[], double &close[], double &open[], double &ma1Values[], double &ma2Values[])
{
    // 检查单根信号K线形态 - 做多
    if (
        // ma1Values[1] < ma2Values[1] &&                       // 均线排列条件
        ((open[1] < ma1Values[1] && close[1] > ma2Values[1]) || (open[1] < ma2Values[1] && close[1] > ma1Values[1])) && // K线形态条件
    ...
    // 检查两根信号K线形态 - 做多
    if (
        // ma1Values[2] < ma2Values[2] && ma1Values[1] < ma2Values[1] &&                                                     // 均线排列条件
        ((open[2] < ma1Values[2] && close[1] > ma2Values[1]) || (open[2] < ma2Values[2] && close[1] > ma1Values[1])) &&                                                              // 第一根信号K线和第二根信号K线形态条件
